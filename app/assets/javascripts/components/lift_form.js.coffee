...
@LiftForm = React.createClass
  ...
  calculateOneRm: ->
    if @state.weightlifted and @state.respsperformed
        @state.onerm = @state.weightlifted / coefficients[@state.respsperformed]
    else
      0
  valid: ->
    @state.date && @state.liftname && @state.weightlifted && @state.respsperformed && @state.onerm
  handleSubmit: (e) ->
    e.preventDefault()
